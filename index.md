Welcome to the PCAPdroid User Guide.

# Contents

- [1. Quick Start](quick_start)
    - [1.1 What is PCAPdroid](quick_start#11-what-is-pcapdroid)
    - [1.2 Basic Setup](quick_start#12-basic-setup)
    - [1.3 App Filter](quick_start#13-app-filter)
- [2. Dump Modes](dump_modes)
    - [2.2 None](dump_modes#2.1-none)
    - [2.3 HTTP Server](dump_modes#22-http-server)
    - [2.4 UDP Exporter](dump_modes#23-udp-exporter)
- [3. TLS Decryption](tls_decryption)
    - [3.1 Decrypt HTTPS/TLS](tls_decryption#31-decrypt-https/tls)
    - [3.2 Decryption after Android 7](tls_decryption#32-decryption-after-android-7)
    - [3.3 Troubleshooting](tls_decryption#33-troubleshooting)
- [4. FAQ](faq)
