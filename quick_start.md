...
*Note: The following information is only relevant to people who need to perform a per-packet analysis based on the PCAP file generated by PCAPdroid. This does not apply to the [Root Capture](https://emanuele-f.github.io/PCAPdroid/advanced_features#44-root-capture).*

In short, if you need to perform a low level analysis on the L3/L4 headers (e.g. on some fields of the TCP header) or on the size/timing of packets, the non-root capture is not appropriate for your task. You should instead use the root capture, which provides a 1:1 copy of the traffic of your device without any modifications. What follows here is a detailed explanation of why this happens.

The non-root mode of PCAPdroid uses the VPNService to capture the network traffic. When the VPNService is active, the traffic is routed to the PCAPdroid app, which implements a L4 proxy to connect it back to the Internet. The VPNService alone alters the traffic because it uses a different subnet and network interface. Moreover, what you see in PCAPdroid or in the PCAP file is the traffic which goes from the app to the VPNService, but you cannot see the traffic which goes from the VPNService over the Internet. Given a connection `saddr1:sport1 -> daddr:dport` (the original) made by an app, PCAPdroid creates a new connection `saddr2:sport2 -> daddr:dport` (the proxied) which goes over the Internet. The PCAP generated by PCAPdroid contains the data for the original connection, but if you capture the data on your router you will see the proxied one instead.

The obvious difference is that the source IP address and port are different. Actually, since the L4 proxy works by using L4 sockets, the whole IP and TCP header may also be different (apart from the destination IP address and port, which is the same). PCAPdroid emulates the TCP features like the TCP window, SEQ and ACK numbers well enough to trick the app into thinking that it's talking to the remote site, whereas in reality it's talking to the L4 proxy. The proxy uses L4 sockets to talk to the remote site, so it doesn't have access to the IP/L4 headers of the proxied connection. Moreover it cannot reliable determine if the data received from the L4 socket was sent as a single packet or multiple packets from the remote site. This is why you cannot reliably use the generated PCAP to perform per-packet analysis.

To recap, the modifications only affect the L3 and L4 layers of the packets, whereas the L7 data is untouched. In particular:

- All packets coming from the Internet contain synthetic IP and TCP/UDP headers. Only the destination IP address and port correspond to the one of the proxied connection.
