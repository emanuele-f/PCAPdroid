...
- Some IP and TCP features may be disabled or altered, e.g. TCP window scaling and TCP keep alive
- Packets coming from the internet are aggregated, so intra-packet/size analysis cannot be performed on them. This also means that packets bigger than MTU may be seen.
