...
*Note: The following information is only relevant to people who need to perform a per-packet analysis based on the PCAP file generated by PCAPdroid. Moreover, this does not apply to the [Root Capture](https://emanuele-f.github.io/PCAPdroid/advanced_features#44-root-capture).*

In order to work in non-root mode, PCAPdroid needs to proxy (at the L4 layer) the connections made by the apps. This introduces some changes in the network traffic, which will be different from the traffic made by the apps when PCAPdroid is not running. So in essence this means that, in order to see the traffic, PCAPdroid needs to modify it.

The modifications only affect the L3 and L4 layers of the packets, whereas the L7 data is untouched. In particular:

- All packets coming from the Internet contain synthetic IP and TCP/UDP headers. In particular, only the IP addresses and the L4 ports correspond to the reality.
...
In conclusion, if you need to perform a low level analysis on the L3/L4 headers or on the size/timing of packets, non-root capture is not appropriate for your task. You should instead use the root capture, which provides a 1:1 copy of the traffic of your device without any modifications.
